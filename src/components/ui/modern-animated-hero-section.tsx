...
const ScrambledTitle: React.FC<ScrambledTitleProps> = ({ 
  ...
  return (
    <span 
      ref={elementRef}
      className="inline-block min-w-[200px] text-gradient"
    >
      {phrases[0]}
    </span>
  )
...
const RainingLetters: React.FC<RainingLettersProps> = ({ className = "" }) => {
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
      ...
      {characters.map((char, index) => (
        <span
          key={index}
          className={`absolute font-mono text-xs transition-colors duration-100 ${
