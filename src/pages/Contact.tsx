...
const Contact = () => {
  return <Layout>
      <PageHeader title="Contact Us" subtitle="Get in touch with our department" badge="Reach Out" />
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <div className="space-y-4">
                {contactInfo.map(info => <a key={info.title} href={info.link || "#"} className={`glass-card border-glow rounded-xl p-6 flex items-start gap-4 hover:glow-box transition-all duration-300 group ${info.link ? "cursor-pointer" : "cursor-default"}`}>
                    ...
                    {info.link && <ExternalLink className="h-4 w-4 text-muted-foreground group-hover:text-primary transition-colors" />}
                  </a>)}
              </div>
            </div>
